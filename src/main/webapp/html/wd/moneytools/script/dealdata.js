...
function checkData(objArray){
	...
	for(var i=0;i<objArray.length;i++){
		...
		if(textnow.value==""){
			rst="false";
			$.icrmDialog.showMessge('提示',"请输入完整！");
			textnow.focus()	;
			textnow.select();
			break;
		}	
...
function isInteger(objArray){
	...
	for(i=0;i<objArray.length;i++){
		...
		if(isNaN(textnow.value)){
			rst="false";
			$.icrmDialog.showMessge('提示',"请输入数字！");
			textnow.focus();
			textnow.select();
			break;
		}	
...
function isTrueYear(objYear){
	...
	if(textnow.value<1900){
		rst="false";
		$.icrmDialog.showMessge('提示',"请输入正确年份！");
		textnow.focus();
		textnow.select();
	}
	return rst;
...
function isTrueMonth(objMonth){
	...
	if(textnow.value>12||textnow.value<1){
		rst="false";
		$.icrmDialog.showMessge('提示',"请输入正确月份！");
		textnow.focus();
		textnow.select();
	}
	return rst;
...
function isTrueDate(objDate){
	...
	if(textnow.value>31){
		rst="false";
		$.icrmDialog.showMessge('提示',"请输入正确日期！");
		textnow.focus();
		textnow.select();
	}
	return rst;
