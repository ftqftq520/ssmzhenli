...
{
  var xmlroot = xmlDoc.documentElement;    
  for (i=0; i<(xmlroot.children.length); i++)//对每一个结点进行处理
  {   
	  if (xmlroot.children[i].children[4].innerHTML == typeid)
	    {
			var oOption = document.createElement("OPTION");
		     
			oOption.innerHTML = GetHowLong(xmlroot.children[i].children[3].innerHTML);
			oOption.value = xmlroot.children[i].children[3].innerHTML;
			combo.options.add(oOption);
	     }
	  ...
	  if (xmlroot.children[i].children[4].innerHTML == typeid)
	    {
			...
			oOption.innerText = GetHowLong(xmlroot.children[i].children[3].innerHTML);
			oOption.value = xmlroot.children[i].children[7].innerHTML;
			combo.options.add(oOption);
	     }
