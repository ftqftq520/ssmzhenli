...
(/** @lends <global> */function( $ ) {
	...
	function _fnColumnOptions( oSettings, iCol, oOptions )
	{
		...
		oCol.fnGetData = function (oData, sSpecific) {
			...
			if(oCol.dataType == "numberic"){
				...
				if(!ICRM.isEmpty(innerData,true)){
					innerData=Number.fmoney(innerData,parseInt(precision));
				}
			}else if(oCol.dataType == "date"){
				var format="yyyy-MM-dd";
				if(typeof(oCol.format) != "undefined"){
					if(!ICRM.isEmpty(oCol.format,true)){
						format=oCol.format;
					}
				}
				if(!ICRM.isEmpty(innerData,true)){
					var dataTemp=new Date(innerData).format(format);
					if(dataTemp == "NaN-aN-aN" ){
						innerData=converToDate(innerData).format(format);
					}else{
						innerData=dataTemp;
					}
				}
			}else if(oCol.dataType == "percent"){
				if(!ICRM.isEmpty(innerData,true)){
					innerData=Number.fmoney(parseFloat(innerData)*100);
				}
			}else if(oCol.dataType == "longtext"){
				var displayNum="30";
				if(typeof(oCol.displayNum) != "undefined"){
					if(!ICRM.isEmpty(oCol.displayNum,true)){
						displayNum=oCol.displayNum;
					}
				}
				if(!ICRM.isEmpty(innerData,true)){
					innerData="<span title='" + innerData + "'>" + ICRM.subString(innerData,parseInt(displayNum)) +"</span>";
				}
			}
	...
	function _fnBuildHead( oSettings )
	{
		...
		for ( i=0, ien=columns.length ; i<ien ; i++ ) {
			column = columns[i];
			cell = $( column.nTh ).addClass( "thead-text-center hidden-480" );
	...
	{
		//edit by YangFan 2014年10月14日 18:45:59
		if(oSettings.autoLoad === false){
			oSettings.autoLoad = true;
		...
		_fnCallbackFire( oSettings, 'aoFooterCallback', 'footer', [ $(oSettings.nTFoot).children('tr')[0],
			_fnGetDataMaster( oSettings ),oSettings.sumData,iDisplayStart, iDisplayEnd, aiDisplay ] );
	...
	DataTable = function( options )
	{
		...
		this.each(function() {
			...
			//edit by YangFan 2014年10月14日 18:38:31
			oSettings.autoLoad = oInit.autoLoad;
